...
const Notifications = () => {
    ...
    return (
        ...
        <Typography variant='h4' textAlign='center' style={{fontWeight:700}} sx={{my:5}}>
            ...
            {
            !unread ? 
                <Button variant='contained' sx={{ml:2}} style={{backgroundColor:"green"}} onClick={handleUnread}>See Unread Messages</Button>:
                <Button variant='contained' sx={{ml:2}}  onClick={handleRead}>See All Messages</Button>
            }
        </Typography>
            ...
            <Table aria-label="customized table">
                ...
                <TableBody>
                    {messages?.map((message, index) => (
                       <Notification key={message._id} 
                            index={index}
                            unread={unread} 
                            message={message} 
                            handleNotification={handleNotification}

                            refetch= {refetch}></Notification>
                    ))}
                </TableBody>
            </Table>
        </TableContainer>
