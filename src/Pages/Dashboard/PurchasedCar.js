...
const PurchasedCar = ({ bookedVehicle, index, refetch }) => {
  ...
  useEffect(() => {
    fetch(
      `https://car-server-d4s0106ne-imranhossain1.vercel.app/review?carId=${carId}&userEmail=${userEmail}`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data) {
  ...
  const url = `https://car-server-d4s0106ne-imranhossain1.vercel.app/vehicle/${carId}`;
            ...
            <>
              <Tooltip title="Edit">
                <EditLocationAltSharpIcon
                  onClick={handlePurchasedCarEditOpen}
                  sx={{ color: "green" }}
                  fontSize="large"
                ></EditLocationAltSharpIcon>
              </Tooltip>
              <Tooltip title="Delete">
                <DeleteIcon
                  onClick={handlePurchasedCarDeleteOpen}
                  sx={{ color: "red", ml: 1 }}
                  fontSize="large"
                ></DeleteIcon>
              </Tooltip>
