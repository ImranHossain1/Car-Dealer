...
const VehicleBooking = () => {
  ...
  const url2 = `https://car-dealer-server-production-4828.up.railway.app/review/${id}`;
  const {
    data: vehicle,
    isLoading,
    refetch,
  ...
  const { data: reviews, isReviewLoading } = useQuery(["reviews", id], () =>
    fetch(url2, {
      method: "GET",
    }).then((res) => res.json())
  );
      ...
      <Box sx={{ flexGrow: 1, my: 3 }} style={{ marginTop: "5%" }}>
        <Container disableGutters>
          <Box>
            ...
            >
              {vehicle.catagory} Car from{" "}
              <span style={{ color: "orange" }}>{vehicle.company}</span>
            </Typography>
            <Grid
              container
              spacing={{ xs: 2, md: 3 }}
              ...
              <Grid item xs={12} md={4}>
                ...
                >
                  <CardContent style={{ textAlign: "justify" }}>
                    ...
                    >
                      Classic{" "}
                      <span style={{ color: "#5535f9", fontWeight: "800" }}>
                        {" "}
                        {vehicle.company}{" "}
                      </span>{" "}
                      Design
                    ...
                    <Typography variant="h6" sx={{ mb: "10px" }}>
                      Cost:{" "}
                      <span style={{ color: "red" }}>${vehicle.cost}</span>
                    </Typography>
                    {vehicle.quantity > 0 ? (
                      <Typography
                        variant="body2"
                    ...
                    {vehicle.quantity > 0 ? (
                      <Link
                        to={`/confirmVehicle/${vehicle._id}`}
                        className={classes.btn}
                        style={{ textDecoration: "none", padding: "10px 40px" }}
                      >
          ...
          <Box sx={{ px: 2 }}>
            ...
            <Typography variant="body2" color=" #566573" sx={{ mb: 2 }}>
              This is a configurator. Industry-wide supply issues impacting
              automotive manufacturing around the world may cause delays or
              non-availability of certain components, features and models.
              Please see your dealer for additional information about your
              chosen {vehicle.company}.
            </Typography>
            {vehicle.quantity > 0 ? (
              <Link
                to={`/confirmVehicle/${vehicle._id}`}
            ...
            <Typography variant="h6">Base MsRP</Typography>
            <Divider></Divider>
            <Box
              sx={{
                display: "flex",
            ...
            >
              <Typography variant="body">
                18" Double-spoke Wheels, Style 796 with All-season Run-flat
                Tires
              </Typography>
              <Typography
                variant="body"
